...
import { Result, Success, Failure } from "../helper/types";
...
export class NotionClientHelper implements ClientHelper {
  ...
  constructor(
    ...
    lifelogDabase: DatabaseId | undefined,
    ...
    if (!restrauntDBId)
      throw new Error("Do not find NOTION_RESTRAUNT_DATABSE_ID");
   ...
   */
  async featchPageIdsFromDB(
    dbId: string,
    properties: object,
  ): Promise<{ id: string; title: string }[]> {
    const filteringQuery: { database_id: string; filter: any } = {
      database_id: dbId,
      filter: properties,
    };
    try {
  ...
  async createPage(
    ...
    if (response) {
      ...
      return new Failure("Failed create page");
    }
  }
}
